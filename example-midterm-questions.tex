...
\usepackage{xcolor}
\usepackage{tabu}  % for colored rows
...
\answer{
    We know the improvement is ``signficant'' -- but, that probably means ``statistically significant'', so: I'd like to know how big was the improvement, in interpretable real-world terms (percent improvement, or an effect size).
}
...
\begin{enumerate}
  ...
  \item What can we conclude about whether the parasite prefers snakes or lizards?
    ...
    \answer{
          ...
          If the difference is real, then based on these data it is probably fairly substantial 
          (snakes have about 10\% more parasites than lizards, effect size of 0.7);
...
\begin{enumerate}
  ...
  \item Test the null hypothesis using a sign test, and briefly state your conclusion.
    ...
    \answer{
      $N_+=26$, $N_-=14$, $B_s=26$, $n_d=40$; 
      since this is beyond Table 7 in the book, we need to use a computer to get: $P=0.04034523$.
      \emph{Note: on the final I wouldn't make you do this outside the table for such a large number.)}
      We have fairly good statistical evidence students are more likely to be in a better mood on Mondays (but, we don't know why).
    }
    % \vspace{8\baselineskip}
  ...
  \item Estimate the proportion of USC students that feel better on Mondays than on Fridays, and give a 95\% confidence interval for this proportion.
    ...
    i.e. from 49.4\% to 77.9\%.\\
    \emph{Note: since this overlaps 50\%, barely, this contradicts part B,
        that found $P<0.05$ against a null of 50\%.
        This difference is because B computes the $P$-value exactly, using the Binomial distribution,
        but the CI here uses the normal approximation.
        On a real test, I would (hopefully!) avoid contradictions like this.
    }
    }
    % \vspace{7\baselineskip}
...
\begin{enumerate}
    ...
    \item Give two reasons why the Wilcoxon Signed-Rank test is a good choice for these data.
        ...
        \answer{We have paired samples; and the Wilcoxon test is better than a paired-sample $t$-test because the distribution of the values is very skewed (non-normal).}

    \item State the null and alternative hypotheses appropriate for carrying out a Wilcoxon Signed-Rank test.

        \answer{
            \begin{align*}
                H_0 &: \text{The distribution of differences in income between husband and wife among LA couples is symmetric.}\\
                H_A &: \text{Either husbands, or wives, amongst LA couples, tend to make have higher incomes than the other.}
            \end{align*}
        }
    ...
    \item Carry out the Wilcoxon Signed-Rank test.
        ...
        \answer{
            From the signed ranks above, $W_+=8+2+7=17$ and $W_-=3.5+6+1+5+9+3.5=28$, so $W_s=28$, and $n_D=9$.
            (As a quick check, $W_++W_-=45=9\times(9-1)/2$.)
            The table gives $P>0.2$.
        }

    \item What can we conclude about whether LA wives or husbands tend to have larger incomes?

        \answer{
            We have no strong statistical evidence that either wives or husbands tend to have higher incomes in LA,
            but our sample size was quite small,
            so there could still be a large effect that we did not detect.
        }
