...
dotgroup <- function (x,f,gm=TRUE,sds=FALSE,col='black',gm.col='black',length=.1,factor=.2,...) {
...
quakes$ddate <- difftime( quakes$date , as.Date("2014-01-1") )
...
par(mar=c(4,3,1,1)+.1)
    with( subset(quakes, MAG>3), {
            plot(MAG, dotjits, xlab="magnitude", ylim=c(0,3), pch=20, yaxt='n', ylab='' )
            # dotgroup.horiz( MAG, rep("2014",length(MAG)), xlab='magnitude', ylims=c(0,2.5) ) 
            segments( y0=dotloc-.25, y1=dotloc+.25, x0=mean(MAG), lwd=2, col='red' )
            segments( y0=dotloc-.25, y1=dotloc+.25, x0=median(MAG), lwd=2, col='red', lty=2 )
        } )
dev.off()
...
par(mar=c(4,4,1,1)+.1)
    with( subset(quakes, MAG>3), {
            dotgroup( MAG, cut(date,breaks="month"), xaxt='n', ylab='Magnitude', pch=20, cex=0.5, gm.col='red' )
...
par(mar=c(4,4,1,1)+.1)
    with( subset(quakes, MAG>3), {
            boxplot( MAG ~ cut(date,breaks="month"), xaxt='n', ylab='Magnitude' )
            axis(1,at=1:12,labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"), las=3)
        } )
dev.off()
